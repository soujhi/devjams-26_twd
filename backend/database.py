...
# Handle Vercel read-only filesystem by writing to /tmp if in Vercel/Serverless environment
IS_VERCEL = os.environ.get("VERCEL", "0") == "1" or os.environ.get("AWS_LAMBDA_FUNCTION_NAME") is not None

ORIG_DB_PATH = os.path.join(os.path.dirname(__file__), "platelet_iq.db")

if IS_VERCEL:
    DB_PATH = "/tmp/platelet_iq.db"
    # Copy pre-populated DB to /tmp if not already there
    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            units_data = [
                ...
                ("P-4437", "B+", 0, "19:10"), ("P-4450", "O+", 0, "16:05"), ("P-4457", "A+", 0, "23:59"),
                # 1 day left (14 units)
